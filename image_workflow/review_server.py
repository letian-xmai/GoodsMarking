...
function setMenu(name){activeMenu=name;menuLabel.classList.toggle('active',name==='label');menuStats.classList.toggle('active',name==='stats');menuImages.classList.toggle('active',name==='images');labelTools.style.display=name==='label'?'block':'none';statsTools.style.display=name==='stats'?'flex':'none';imageTools.style.display=name==='images'?'flex':'none';imageMetrics.style.display=name==='images'?'flex':'none';}
...
function renderImageMetrics(m){allImageCount.textContent=m.total_images||0;modelFinalImageCount.textContent=m.model_final_images||0;qualifiedImageCount.textContent=m.qualified_images||0;}
