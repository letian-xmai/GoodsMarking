...
document.querySelectorAll('.tab').forEach(x=>x.onclick=()=>{activeView=x.dataset.view;msg.textContent='';renderGrid();});menuLabel.onclick=()=>loadProduct();menuStats.onclick=()=>loadStats(1);searchStats.onclick=()=>loadStats(1);statsSearch.onkeydown=e=>{if(e.key==='Enter')loadStats(1);};prevPage.onclick=()=>loadStats(statsPage-1);nextPage.onclick=()=>loadStats(statsPage+1);submit.onclick=submitBatch;reload.onclick=()=>activeMenu==='stats'?loadStats(statsPage):loadProduct(product&&product.outward_code);const initialCode=new URLSearchParams(location.search).get('outward_code');loadProduct(initialCode);
</script></body></html>"""
